...
static void draw_lines(SDL_Surface *windows_surface, Uint32 color,
  ...
  for (size_t position = 0; position < limit; ++position) {
    SDL_Rect line = (SDL_Rect){position * x_init_point, position * y_init_point,
...
void draw_grid(SDL_Surface *windows_surface) {
  // Draw columns
  draw_lines(windows_surface, COLOR_GRAY, CELL_SIZE, 0, COLUMNS, SCREEN_HEIGHT,
             GRID_LINE_THICKNESS);
  // Draw rows
  draw_lines(windows_surface, COLOR_GRAY, 0, CELL_SIZE, ROWS, SCREEN_WIDTH,
             GRID_LINE_THICKNESS);
}
...
void draw_particle(SDL_Surface *window_surface, TCell cell) {
  ...
  SDL_Rect filled_area =
      (SDL_Rect){cell.x,
                 // Offset the initial pixel of the filled_area by the void part
                 // of the cell
                 cell.y + void_size, cell.size, CELL_SIZE * cell.fill_level};
